...
export function NotificationSounds() {
  ...
  return (
    // Dos columnas desde xl, partidas por semántica: izquierda «¿me interrumpen y
    // cuánto?» (avisos, mute, volumen — la dependencia mute→volumen queda junta),
    // derecha «¿cómo suena cada cosa?» (los tres timbres y el escalamiento).
    <div className="grid gap-8 xl:grid-cols-2 xl:gap-x-10">
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-[0.7rem] text-muted-foreground">
              Las tarjetas que aparecen en la esquina. Si las apagas, las notificaciones
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-[0.7rem] text-muted-foreground">
              Si lo apagas, las notificaciones siguen apareciendo en la campana, sin
              sonar.
            </p>
          </div>
          <Switch
        ...
        <div className="space-y-2">
          ...
          <p className="text-[0.7rem] text-muted-foreground">
            Para trabajar en un local con gente: bájalo en vez de apagarlo.
          </p>
      ...
      <div className="space-y-6">
        {SLOTS.map(({ slot, label, hint }) => {
          ...
          const taken = new Set(
            SLOTS.filter((entry) => entry.slot !== slot).map(
              (entry) => assignments[entry.slot],
            ),
          );
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger id={`sound-${slot}`} className="w-full">
                    <SelectValue placeholder="Elige un sonido" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      {options.map((choice) => (
        ...
        <div className="space-y-2">
          {/* <p>, no <Label>: no hay control que etiquetar (el botón lleva su
              propio aria-label) y un label huérfano es un falso affordance. */}
          <p className="text-sm leading-none font-medium">Escalamiento a una persona</p>
          <p className="text-[0.7rem] text-muted-foreground">
