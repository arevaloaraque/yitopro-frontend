...
function stepForMissingKey(key: string): OnboardingStep {
  ...
  if (k.includes("schedule") || k.includes("horario") || k.includes("hour")) return 3;
...
export function StepConfirmar() {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      // `complete()` already handles the 400 (missing steps) case; anything
      // else (5xx, network failure) lands here — show it so the user can retry.
      setCompleteError(
