...
| reports                  | `getValueSummary` + `exportValueCsv` (el CSV llega como TEXTO vía el `parseBody` no-JSON del client); Decimales string→number; ventana `date_from`/`date_to` obligatoria; moneda por fila (`formatPrice`); solo owner — el backend responde 403 al staff                                                                                                                                                                                                                                                                                                        |
...
- **Reportes** (`/reports`, `components/reports`, `lib/api/reports.ts`): pantalla de valor solo-owner —
  núcleo universal (respuestas ai/operador, handoffs, tiempo de respuesta, fuera de horario, actividad
  IA) + bloques de citas/pedidos/cobros gateados por actividad de por vida (bloque `null` = no se dibuja
  nada), y export CSV de los cobros de la ventana. El staff es redirigido; un 403 del backend se muestra
  tal cual.
...
- **AWS Amplify Hosting** — `amplify.yml` (Next.js SSR / WEB*COMPUTE). El `preBuild` fija Node 24 e
  ...
  `NEXT_PUBLIC_META*_`) — no van en el repo (`.env_`está gitignored salvo`.env.example`).
